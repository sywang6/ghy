...
//对selectionKey执行cancel()方法后的效果,该键都被添加到其选择器的已取消键集中.取消某个键会导致在下一次select()方法选择操作期间注销该键的通道
//,而在注销是将从所有选择器的键集中移除该键
public class Test1 {
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        Thread client = new Thread(){
          public void run(){
              try {
                  ...
                  outputStream1.write("我是中国人,我来自客户端同7777!".getBytes());
                  socket1.close();
        ...
        Thread getInfo = new Thread(){
          public void run(){
              try {
                  Thread.sleep(10000); //阻塞10秒,让服务端先处理客户端请求
                  System.out.println();
                  Set<SelectionKey> keys = selector.keys();
                  Set<SelectionKey> selectionKeys = selector.selectedKeys();
                  System.out.println("select()方法执行第2次之后的信息:");
            ...
            while (iterator.hasNext()){
                ...
                if(key.isAcceptable()){
                    ...
                    if(serverSocket.getLocalPort() == 7777 ){
                        //该key添加到选择器的已取消键集中,  下一次select()方法注销该键的通道,并从选择器的所有键集中移除该键
                        key.cancel();
                        AbstractSelectionKey abc;
